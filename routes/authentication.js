...
module.exports = (router) => {
      ...
      if (!req.body.username) {
        ...
        if (!req.body.password) {
          ...
          if(!req.body.studentName){
            res.json({ success: false, message: 'You must provide a studentName' }); // Return error
          } else {
            if(!req.body.department){
              res.json({ success: false, message: 'you must provide a department name'});// return error
            } else {
              if (!req.body.career) {
                res.json({success: false, message: 'Enter a Career'});
              } else {
                  ...
                  if (err.errors.email) {
                    ...
                    if (err.errors.username) {
                      ...
                      if (err.errors.password) {
                        ...
                        if (err.errors.studentName) {
                          res.json({success: false, message: err.errors.studentName.message});//Return error
                        } else {
                          if (err.errors.department) {
                            res.json({success: false, message: err.errors.department.message});// return error
                          } else {
                            if (err.errors.career){
                              res.json({success:false, message:err.errors.career.message});
                            } else {
  ...
  router.get('/profile', (req, res) => {
    ...
    User.findOne({ _id: req.decoded.userId }).select('username email studentName department career').exec((err, user) => {
  ...
  router.get('/publicProfile/:username', (req, res) => {
    ...
    if (!req.params.username) {
      ...
      User.findOne({ username: req.params.username }).select('username email studentName department career').exec((err, user) => {
        // Check if error was found
        if (err) {
