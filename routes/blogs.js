			...
			if (!req.body.professorName) {
				...
				if (!req.body.body) {
					...
					if (!req.body.createdBy) {
						res.json({
							success: false,
							message: 'Review added by is required.'
						}); // Return error
					} else {
						// Create the blog object for insertion into database
						const blog = new Blog({
							title: req.body.title, // Title field
							professorName: req.body.professorName, //professorName
							body: req.body.body, // Body field
							createdBy: req.body.createdBy // CreatedBy field
						});
							...
							if (err) {
								...
								if (err.errors) {
									...
									if (err.errors.title) {
										...
										if (err.errors.professorName) {
											res.json({
												success: false,
												message: err.errors.professorName.message
											}); // Return error message
										} else {
											// Check if validation error is in the body field
											if (err.errors.body) {
  ...
  router.get('/allBlogs', (req, res) => {
    ...
    Blog.find({}, (err, blogs) => {
      ...
      if (err) {
        ...
        if (!blogs) {
          res.json({ success: false, message: 'No reviews found.' }); // Return error of no blogs found
        } else {
            ...
            User.findOne({ _id: req.decoded.userId }, (err, user) => {
              ...
              if (err) {
                ...
                if (!user) {
                  ...
                  if (user.username !== blog.createdBy) {
                    res.json({ success: false, message: 'You are not authorized to edit this Review .' }); // Return error message
                  } else {
                    // blog.title = req.body.title; // Save latest blog title
                    // blog.professorName = req.body.professorName;// Save latest professorName
                    ...
                    blog.save((err) => {
                      if (err) {
                        ...
                        res.json({ success: true, message: 'Review Successfully Updated!' }); // Return success message
                      }
                    });
                  }
                }
                  ...
                  if (user.username !== blog.createdBy) {
                    ...
                    blog.remove((err) => {
                      if (err) {
                        ...
                        res.json({ success: true, message: 'Review deleted Successfully!' }); // Return success message
                      }
                    });
                  }
                }
    ...
    if (!req.body.id) {
      ...
      Blog.findOne({ _id: req.body.id }, (err, blog) => {
        ...
        if (err) {
          ...
          if (!blog) {
            res.json({ success: false, message: 'That selected review was not found.' }); // Return error message
          } else {
    ...
    if (!req.body.id) {
      ...
      Blog.findOne({ _id: req.body.id }, (err, blog) => {
        ...
        if (err) {
          ...
          if (!blog) {
            res.json({ success: false, message: 'The selected Review was not found.' }); // Return error message
          } else {
            ...
            User.findOne({ _id: req.decoded.userId }, (err, user) => {
              ...
              if (err) {
                ...
                if (!user) {
                  ...
                  if (user.username === blog.createdBy) {
                    res.json({ success: false, messagse: 'Cannot dislike your own review.' }); // Return error message
                  } else {
                    // Check if user who disliked post has already disliked it before
                    if (blog.dislikedBy.includes(user.username)) {
                      res.json({ success: false, message: 'You already disliked this review.' }); // Return error message
                    } else {
